...
- **加入好友的歡迎訊息：停用** ← webhook 自己會回，內含綁定指引
- **Webhook：啟用**
- **自動回應訊息**：要用常見問題功能就啟用，並照下面「常見問題」一節的規則設定

⚠️ **絕對不要建立「沒有設定關鍵字」的自動回應。** LINE 會把那種訊息對**所有**輸入發送——包含綁定碼。使用者會同時收到它和我們的「綁定成功」，變成兩則訊息。
...
**這一整套不需要碰程式，也不需要部署** —— 全部在 Official Account Manager 設定，下一屆自己就能改。

#### 誰回應什麼

bot 的回覆分成兩邊，**互不重疊**，這是不會出現重複訊息的前提：

| 使用者傳的內容 | 誰回應 |
|---|---|
| 8 碼綁定碼格式 | **我們的 webhook**（綁定成功／碼無效） |
| 常見問題關鍵字 | **LINE 的自動回應** |
| 其他任何文字 | **沒有人回**（使用者有圖文選單可按） |

程式那邊只回應「看起來像綁定碼」的訊息，其餘一律安靜跳過（`src/backend/config/line.js` 的 `classifyMessage`）。
...
- 版型任選（3 格或 6 格）
- 圖片可用 OA Manager 內建的簡易編輯器直接做，不必自己開設計軟體
- **每一格的動作選「傳送文字」**，文字填該題的關鍵字（例如 `如何繳交系學會費`）

按下按鈕 = 使用者送出那段文字 → 觸發下面的關鍵字自動回應。
...
- 關鍵字填圖文選單按鈕送出的那段文字
- 比對**不分大小寫與全形半形**；單則訊息最多 51 個關鍵字，可以把同義詞都列上（「會費」「系學會費」「繳費」…）
- 每則訊息最多 5 個對話框

#### 改東西時要注意

圖文選單的按鈕文字與自動回應的關鍵字**必須一致**。只改其中一邊的話，按鈕按下去會沒有任何反應（webhook 會安靜跳過，因為那不是綁定碼）——不會報錯，只是沒反應。
