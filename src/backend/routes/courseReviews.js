            ...
            const order =
                sortBy === 'highest' || sortBy === 'lowest'
                    ? [
                          [
                              sequelize.literal(
                                  '(quality + difficulty + sweetness + usefulness) / 4',
                              ),
                              sortBy === 'highest' ? 'DESC' : 'ASC',
                          ],
                      ]
                    : [['created_at', 'DESC']];
...
router.post(
    ...
    [
        ...
        body('professor')
            .trim()
            .notEmpty()
            .withMessage({ code: 'PROFESSOR_REQUIRED', message: '授課教授為必填' }),
        body('year')
        ...
        body('quality')
            ...
            .withMessage({ code: 'QUALITY_RANGE', message: '課程品質須為0.5-5' }),
        body('difficulty')
            .isFloat({ min: 0.5, max: 5 })
            .withMessage({ code: 'DIFFICULTY_RANGE', message: '難易度須為0.5-5' }),
        body('sweetness')
            .isFloat({ min: 0.5, max: 5 })
            .withMessage({ code: 'SWEETNESS_RANGE', message: '給分高低須為0.5-5' }),
        body('usefulness')
            .isFloat({ min: 0.5, max: 5 })
            .withMessage({ code: 'USEFULNESS_RANGE', message: '實用性須為0.5-5' }),
        // 內容欄位的門檻集中在 config/reviewContent.js。新增時五個欄位全部必填——
        // 回饋金是 100 元，舊的「55 個字」門檻是在還沒有金錢誘因時訂的。
        body('courseContent')
            .trim()
            .isLength(CONTENT_LIMITS.courseContent)
            .withMessage(contentError('COURSE_CONTENT', '課程內容', 'courseContent')),
        body('teachingMethod')
            .trim()
            .isLength(CONTENT_LIMITS.teachingMethod)
            .withMessage(contentError('TEACHING_METHOD', '教學方式', 'teachingMethod')),
        body('assignmentExamFormat')
            .trim()
            .isLength(CONTENT_LIMITS.assignmentExamFormat)
            .withMessage(
                contentError('ASSIGNMENT_EXAM_FORMAT', '作業與考試形式', 'assignmentExamFormat'),
            ),
        body('gradingBreakdown')
            .trim()
            .isLength(CONTENT_LIMITS.gradingBreakdown)
            .withMessage(contentError('GRADING_BREAKDOWN', '評分佔比', 'gradingBreakdown')),
        body('comment')
            .trim()
            .isLength(CONTENT_LIMITS.comment)
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (reviewCount >= MAX_REVIEWS_PER_USER) {
                return res
                    .status(400)
                    .json(errorResponse('REVIEW_LIMIT_EXCEEDED', '您的評價數量已達上限'));
            }
            ...
            if (existingReview) {
                return res
                    .status(400)
                    .json(errorResponse('DUPLICATE_REVIEW', '您已評價過此課程（同學期、同教授）'));
            }
...
router.put(
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (wasRejected) {
                notificationService
                    .notifyReviewResubmitted(review)
...
router.get(
    ...
    async (req, res) => {
        try {
            ...
            const eligibleIds = await getEligibleReviewIds(
                reviews.map((r) => ({
                    courseCode: r.courseCode,
                    professor: r.professor,
                    year: r.year,
            ...
            res.setHeader(
                'Content-Disposition',
                `attachment; filename="course-review-payouts-${new Date().toISOString().slice(0, 10)}.csv"`,
            );
...
router.patch(
    ...
    requirePermission('courseReviews.payout'),
    [
        body('payoutStatus').isIn(PAYOUT_STATUSES).withMessage({
            code: 'PAYOUT_STATUS_INVALID',
            message: '發放狀態須為未處理、已發放或不發放',
        }),
    ],
    async (req, res) => {
        const errors = validationResult(req);
        ...
        try {
            ...
            if (review.status !== 'approved') {
                return res
                    .status(400)
                    .json(
                        errorResponse('PAYOUT_REQUIRES_APPROVED', '只有已核准的評價才能標記發放'),
                    );
            }
...
router.patch(
    ...
    [
        body('status')
            ...
            .withMessage({ code: 'STATUS_INVALID', message: '狀態須為 approved 或 rejected' }),
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (status === 'rejected' && !rejectReason?.trim()) {
                return res
                    .status(400)
                    .json(errorResponse('REJECT_REASON_REQUIRED', '拒絕時請填寫拒絕原因'));
            }
